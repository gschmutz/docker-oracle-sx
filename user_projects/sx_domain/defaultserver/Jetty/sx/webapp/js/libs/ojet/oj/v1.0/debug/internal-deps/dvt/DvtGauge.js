...
DvtGauge.prototype.render = function(options, width, height) 
{
  ...
  // WAI-ARIA Support: If rooted at the stage, then the aria role should only be 'application' if gauge is editable.
  if (!this._bStaticRendering) {
    var bParentIsStage = (this.getParent() == this.getCtx().getStage());
    var tooltip = DvtGaugeRenderer.getTooltipString(this);
    if (this.Options['readOnly'] && bParentIsStage) {
      // JAWS doesn't seem to read aria properties unless they are supported by the role type so we must set a role
 ...
 * @extends {DvtRect}
 */
var DvtStatusMeterGaugeIndicator = function(gauge, context, x1, x2, y1, y2, roundCorners) {
